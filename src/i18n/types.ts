...
export interface Translations {
  ...
  app: {
    ...
    nav: {
      analytics: string;
      chat: string;
      /** Optional: see the note on the session-finder keys. */
      files?: string;
      /** Optional: see the note on the session-finder keys. */
      appearance?: string;
      config: string;
  ...
  sessions: {
    /**
     * Session-finder strings. Optional because every locale but `ar` declares
     * a complete `Translations` object rather than going through
     * `defineLocale`, so a required key here would break all 17 of them and
     * the only quick fix would be pasting English into them. Optional lets a
     * translator add these when they get to them; until then the call sites
     * fall back to English, which is what `defineLocale` would have done.
     */
    findSession?: string;
    findSessionPlaceholder?: string;
    findSessionConfirm?: string;
    findSessionEmpty?: string;
    findSessionLastActive?: string;
    title: string;
    history: string;
    overview: string;
    filterChats: string;
  ...
  // ── Skills page ──
  skills: {
    /** Finder strings — optional for the same reason as the session ones:
     *  every locale but `ar` declares a complete object, so a required key
     *  would break all of them. Call sites fall back to English. */
    findSkill?: string;
    title: string;
    searchPlaceholder: string;
    enabledOf: string;

...
interface Props {
  ...
  /**
   * Chat-mode only: render a link out to the Models page.
   *
   * This picker sets the main model for one chat. Auxiliary Tasks and Mixture
   * of Agents live on the Models page and have no representation here, so
   * without a way out this dialog is a dead end for them — which is exactly
   * why the sidebar kept a Models pill. Passing this replaces that pill.
   *
   * Optional on purpose: ModelsPage opens this same dialog, and a link back to
   * the page you are already on would be nonsense. Callers that ARE the Models
   * page simply omit it.
   */
  ...
  /**
   * Open with a provider and model already chosen, skipping straight to the
   * confirm step. The model finder uses this to hand off a row the user picked
   * by search, so the persist checkbox and the expensive-model confirmation
   * still get shown rather than being bypassed. Omitted, the picker behaves
   * exactly as before.
   */
...
export function ModelPickerDialog(props: Props) {
  ...
  const applyOptions = (r: ModelOptionsResponse) => {
    ...
    setSelectedSlug((prev) => {
      if (initialProvider && next.some((p) => p.slug === initialProvider)) {
    ...
    standalone
      ...
      : (gw as GatewayClient).request<ModelOptionsResponse>(
          "model.options",
          {
            ...
            // No `include_unconfigured` here. This path is the composer's
            // chooser, and asking for the full universe listed 48 providers of
            // which 45 had zero models — Nous Portal, Fireworks, OpenRouter and
            // the rest, none of them selectable. Picking from a list is not the
            // same job as setting one up: the Models page still opts in through
            // `loader`/`api.getModelOptions`, because you cannot add a provider
            // you cannot see.
          },
        );
  ...
  // modal's own layer is scoped to z-2 and the sidebar covers its left

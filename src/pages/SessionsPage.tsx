...
import { CommandPanelTrigger } from "@/components/CommandPanelTrigger";
    ...
    <>
      ...
      {resumeInChatEnabled && (
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/chat-web?resume=${encodeURIComponent(session.id)}`);
          }}
        >
          <Play />
        </Button>
...
const PAGE_SIZE = 20;
/** One fetch backs the finder. 100 is the server's ceiling — `GET
 *  /api/sessions` declares `limit: int = Query(20, ge=0, le=100)` and 422s
 *  anything larger, which is how the first attempt at 500 failed. */
const FINDER_LIMIT = 100;
...
export default function SessionsPage() {
  ...
  // The finder searches every session, not just the page you are on and not
  // just the source chips you have selected — a shortcut that silently hides
  // the session you are hunting for is worse than no shortcut.
  const [finderOpen, setFinderOpen] = useState(false);
  const [finderSessions, setFinderSessions] = useState<SessionInfo[] | null>(null);
  ...
  /**
   * Sessions load lazily on first open and are then reused: the panel is a
   * shortcut, and making it wait on a round trip every time defeats that. A
   * page refresh is what re-reads the list.
   */
  const finderFetched = useRef(false);
  const openFinder = useCallback(() => {
    setFinderOpen(true);
    // Guarded by a ref, not by reading state inside a setState updater —
    // updaters must stay pure, and React calls them twice in development.
    if (finderFetched.current) return;
    finderFetched.current = true;
    api
      .getSessions(FINDER_LIMIT, 0)
      .then((resp) => setFinderSessions(resp.sessions))
      .catch(() => setFinderSessions([]));
  }, []);

  useCommandPanelKey(openFinder);

  const finderItems = useMemo<PanelItem[]>(
    () =>
      (finderSessions ?? []).map((sess) => ({
        id: sess.id,
        title: sess.title?.trim() || t.sessions.untitledSession,
        badge: sess.source ?? undefined,
        meta: sess.message_count
          ? `${sess.message_count} ${t.common.msgs}`
          : undefined,
        group: monthGroup(sess.started_at),
        // The id is how you find a session that never got a title, and it is
        // too long to put on the row.
        keywords: `${sess.id} ${sess.model ?? ""} ${sess.preview ?? ""}`,
      })),
    [finderSessions, t],
  );

  /**
   * Open a stored session in the embedded chat.
   *
   * Origin does not matter. `session.resume` reads the session store rather
   * than the live registry, so a transcript recorded by the CLI or the desktop
   * app reopens here the same way a web one does — the CLI / Desktop badge on
   * the card describes where it came from, not where it can go.
   */
  const openSessionInChat = useCallback(
    (sessionId: string) => {
      if (resumeInChatEnabled) {
        pageNavigate(`/chat-web?resume=${encodeURIComponent(sessionId)}`);
        return;
      }
      // No embedded chat to hand off to, so narrow the page itself to the one
      // session instead of dropping the user back where they started.
      updateSearch(sessionId);
    },
    [resumeInChatEnabled, pageNavigate, updateSearch],
  );

  const chooseFinderSession = useCallback(
    (item: PanelItem) => openSessionInChat(item.id),
    [openSessionInChat],
  );
  ...
  return (
    <div className="flex min-w-0 w-full max-w-full flex-col gap-4">
      ...
      <CommandPanel
        ...
        title={t.sessions.findSession ?? "Find a session"}
        placeholder={
          t.sessions.findSessionPlaceholder ?? "Search by title, id, or model..."
        }
        items={finderItems}
        loading={finderSessions === null}
        emptyLabel={t.sessions.findSessionEmpty ?? "No sessions match that search"}
        confirmLabel={t.sessions.findSessionConfirm ?? "resume"}
        onSelect={chooseFinderSession}
        renderDetail={(item) => {
          const sess = (finderSessions ?? []).find((x) => x.id === item.id);
          if (!sess) return null;
          return (
            <div className="flex flex-col gap-3 text-sm">
              <p className="text-[var(--text-primary)]">{item.title}</p>
              {sess.preview && (
                <p className="line-clamp-6 text-[var(--text-secondary)]">
                  {sess.preview}
                </p>
              )}
              <dl className="flex flex-col gap-1 text-xs text-[var(--text-secondary)]">
                <DetailRow label={t.sessions.sourceFilter} value={sess.source ?? "-"} />
                <DetailRow label="Model" value={sess.model ?? "-"} />
                <DetailRow
                  label={t.common.msgs}
                  value={String(sess.message_count)}
                />
                <DetailRow
                  label={t.sessions.findSessionLastActive ?? "Last active"}
                  value={sess.last_active ? timeAgo(sess.last_active) : "-"}
                />
                <DetailRow label="ID" value={sess.id} mono />
              </dl>
            </div>
          );
        }}
      ...
      {/* The session-count strip (Total / Active in store / Archived /
          Messages / Sources) used to sit here. Removed on request: the page
          is a list you came to read, and five running totals above it were
          bookkeeping nobody acted on. `stats` itself stays — the source
          filter below is built from `stats.by_source`. */}
      ...
      {(showOverviewTab && !isSearching) || showList ? (
        <div className="flex w-full min-w-0 flex-wrap items-center gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
            ...
            <CommandPanelTrigger onClick={openFinder} className="h-8">
              {t.sessions.findSession ?? "Find a session"}
            </CommandPanelTrigger>
            ...
            <Card className="min-w-0 max-w-full overflow-hidden">
              ...
              <CardContent className="grid min-w-0 gap-3">
                {recentSessions.map((s) => (
                  // A button, not a div with a handler: the whole card is the
                  // hit target, so it has to be focusable and answer Enter.
                  // `text-left` because a button centres its content by
                  // default and this one is a record, not a label.
                  <button
                    key={s.id}
                    type="button"
                    onClick={() => openSessionInChat(s.id)}
                    aria-label={`Open ${s.title ?? t.common.untitled} in chat`}
                    className={cn(
                      "flex min-w-0 max-w-full flex-col gap-2 border border-border p-3 text-left",
...
function DetailRow({
  ...
  return (
    <div className="flex gap-2">
      ...
      <dd
        className={`min-w-0 flex-1 truncate text-[var(--text-primary)] ${mono ? "font-mono" : ""}`}
      >
        {value}
      </dd>
    </div>
